...
class MainActivity : AppCompatActivity() {
    ...
    private fun showAlertDialog() {
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (title.trim().isEmpty()) {
                Toast.makeText(this, "Map needs to have a non-empty title", Toast.LENGTH_LONG).show()
    ...
    private fun deserializeUserMaps(context: Context) : List<UserMap> {
        Log.i(TAG, "derserializeUserMaps")
        val data = getDataFile(context)
        if(!data.exists()){
            Log.i(TAG, "Data file doesn't exist")
            return emptyList()
        }
        ObjectInputStream(FileInputStream(data)).use { return it.readObject() as List<UserMap> }
    }

    private fun getDataFile(context: Context) : File {
        Log.i(TAG, "Getting data from ${context.filesDir}")
